...
int Netracker::netracker_chain_handler(kis_packet *in_pack) {
		...
		if (packinfo->distrib == distrib_adhoc && net->type == network_ap) {
			...
			if (globalreg->alertracker->PotentialAlert(alert_adhoc_ref)) {
				...
				globalreg->alertracker->RaiseAlert(alert_adhoc_ref, in_pack, 
												   ...
												   packinfo->channel, al);
				#ifdef PRELUDE
				globalreg->preludetracker->RaiseHeavyAlert(alert_adhoc_ref, in_pack,
												   packinfo->bssid_mac,
												   packinfo->source_mac,
												   packinfo->dest_mac,
												   packinfo->other_mac,
	...
	if (packinfo->type == packet_management && 
		...
		if (alert_airjackssid_ref >= 0 && packinfo->ssid == "AirJack" &&
			...
			#ifdef PRELUDE
			globalreg->preludetracker->RaiseHeavyAlert(alert_airjackssid_ref, in_pack,
											   packinfo->bssid_mac,
											   packinfo->source_mac,
											   packinfo->dest_mac,
											   packinfo->other_mac,
		...
		if (packinfo->cryptset == 0) {
			if (adssid->cryptset && alert_wepflap_ref &&
				...
				#ifdef PRELUDE
				globalreg->preludetracker->RaiseHeavyAlert(alert_wepflap_ref, in_pack,
												   packinfo->bssid_mac,
												   packinfo->source_mac,
												   packinfo->dest_mac,
												   packinfo->other_mac,
		...
		if (alert_chan_ref >= 0 && newnetwork == 0 && net->channel != 0 &&
			...
			#ifdef PRELUDE
			globalreg->preludetracker->RaiseHeavyAlert(alert_chan_ref, in_pack,
											   packinfo->bssid_mac,
											   packinfo->source_mac,
											   packinfo->dest_mac,
											   packinfo->other_mac,
	...
	if (alert_bcastdcon_ref >= 0 && packinfo->type == packet_management &&
		...
		#ifdef PRELUDE
		globalreg->preludetracker->RaiseHeavyAlert(alert_bcastdcon_ref, in_pack, 
                                                                                   packinfo->bssid_mac, 
                                                                                   packinfo->source_mac, 
                                                                                   packinfo->dest_mac, 
                                                                                   packinfo->other_mac, 
	...
	if (newnetwork) {
		...
		*/
		#ifdef PRELUDE
		globalreg->preludetracker->RaiseDetectNetwork(nettype.c_str(), ssid.c_str(), net, packinfo);
		#endif
...
int Netracker::datatracker_chain_handler(kis_packet *in_pack) {
	...
	if (datainfo->proto  == proto_dhcp_discover) {
		...
		if (cli->dhcp_host != datainfo->discover_host &&
			...
			#ifdef PRELUDE
			globalreg->preludetracker->RaiseHeavyAlert(alert_dhcpname_ref, in_pack, 
                                                                                           packinfo->bssid_mac, 
                                                                                           packinfo->source_mac, 
                                                                                           packinfo->dest_mac, 
                                                                                           packinfo->other_mac, 
		...
		if (cli->dhcp_vendor != datainfo->discover_vendor &&
			...
			#ifdef PRELUDE
			globalreg->preludetracker->RaiseHeavyAlert(alert_dhcpos_ref, in_pack, 
                                                                                           packinfo->bssid_mac, 
                                                                                           packinfo->source_mac, 
                                                                                           packinfo->dest_mac, 
                                                                                           packinfo->other_mac, 
	...
	if ((packinfo->source_mac == net->bssid && 
		...
		if (datainfo->proto  == proto_dhcp_offer) {
			...
			if (alert_dhcpcon_ref >= 0 && 
				...
				#ifdef PRELUDE
				globalreg->preludetracker->RaiseHeavyAlert(alert_dhcpcon_ref, in_pack,
                                                                                                   packinfo->bssid_mac,
                                                                                                   packinfo->source_mac,
                                                                                                   packinfo->dest_mac,
                                                                                                   packinfo->other_mac,
	...
	if (packinfo->type == packet_management &&
		...
		for (unsigned int x = 0; x < apspoof_vec.size(); x++) {
			...
			if (match && globalreg->alertracker->PotentialAlert(alert_adhoc_ref)) {
				...
				#ifdef PRELUDE
				globalreg->preludetracker->RaiseHeavyAlert(alert_ssidmatch_ref, in_pack, 
                                                                                                   packinfo->bssid_mac, 
                                                                                                   packinfo->source_mac, 
                                                                                                   packinfo->dest_mac, 
                                                                                                   packinfo->other_mac, 
